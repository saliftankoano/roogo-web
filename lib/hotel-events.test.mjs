...
describe("hotel event codes", () => {
  ...
  it("matches listing city IDs with hotel and event labels", () => {
    assert.equal(normalizeHotelEventCity("ouaga"), "ouagadougou");
    assert.equal(normalizeHotelEventCity("Ouagadougou"), "ouagadougou");
    assert.equal(normalizeHotelEventCity("bobo"), "bobo-dioulasso");
    assert.equal(normalizeHotelEventCity("Bobo-Dioulasso"), "bobo-dioulasso");
    assert.equal(normalizeHotelEventCity("Pô"), "po");
  });

  it("treats the event end as the inclusive final stay night", () => {
    assert.equal(eventCheckoutDate("2026-09-12"), "2026-09-13");
    assert.equal(
      isBookingWithinEvent(
        "2026-09-12",
        "2026-09-13",
