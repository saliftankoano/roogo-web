...
const mocks = {
  ...
  "@/lib/daily-bookings":
    "export const finalizeDailyBookingAfterPayment = async () => ({finalized: !globalThis.__paymentCallbackTest.dailyBlocked}); export const isBlockedDailyFinalize = result => !result.finalized;",
  "@/lib/africastalking":
  ...
  "@/lib/property-lock-finalization": `export const isMonthlyProperty = async () => false;
     export const finalizeMonthlyPropertyLock = async () => ({ paymentStatus: 'completed', fulfillmentConflict: false });
     export const notifyMonthlyPropertyLockConflict = async () => {
       const s = globalThis.__paymentCallbackTest;
       s.escalations++;
       if (s.escalationError) throw new Error('Context unavailable');
     };`,
};
state.db = {
  from(table) {
    ...
    const execute = (single) => {
      if (!patch) {
        state.reads++;
        if (state.initialReadError && state.reads === 1)
...
const { POST: checkVisit3dStatus } =
  await import("../app/api/visites-3d/status/route.ts");
...
beforeEach(() => {
  state.initialReadError = false;
  state.dailyBlocked = false;
  state.escalations = 0;
  state.escalationError = false;
...
test("lookup outages are not classified as a definitively missing payment", async () => {
  state.initialReadError = true;
  assert.equal((await checkPaymentStatus(statusRequest())).status, 503);
  state.initialReadError = false;
  state.transaction = null;
  assert.equal((await checkPaymentStatus(statusRequest())).status, 404);
});

for (const visit3d of [false, true]) {
  const label = visit3d ? "3D" : "transaction";
  const check = visit3d ? checkVisit3dStatus : checkPaymentStatus;
  for (const [scenario, response] of [
    [
      "HTML 404",
      () => new Response("<html>Route not found</html>", { status: 404 }),
    ],
    ["JSON 404", () => Response.json({ status: "NOT_FOUND" }, { status: 404 })],
    ...[400, 401, 403, 429].map((status) => [
      `lookup HTTP ${status}`,
      () => Response.json({ status: "NOT_FOUND" }, { status }),
    ]),
    [
      "failed lookup 500",
      () => Response.json({ status: "NOT_FOUND" }, { status: 500 }),
    ],
    [
      "terminal-looking 503",
      () =>
        Response.json(
          { status: "FOUND", data: { status: "FAILED" } },
          { status: 503 },
        ),
    ],
    ["malformed 200", () => new Response("<html>Unavailable</html>")],
    [
      "interrupted body",
      () => ({
        ok: true,
    ...
    test(`${label} ${scenario} preserves the deposit for a later completion`, async (t) => {
      state.transaction.status = "submitted";
      state.booking.payment_status = "submitted";
      if (visit3d) state.transaction = null;
      const original = structuredClone(
        visit3d ? state.booking : state.transaction,
      );
      t.mock.method(globalThis, "fetch", async () => response());
      const result = await check(statusRequest());
      const body = await result.json();
      assert.notEqual(body.status, "FAILED");
      assert.notEqual(body.status, "NOT_FOUND");
      // Upstream HTTP 404 must not masquerade as a missing Roogo record.
      assert.notEqual(result.status, 404);
      assert.deepEqual(visit3d ? state.booking : state.transaction, original);
      assert.equal(state.queued.length, 0);
      assert.equal((await callback("COMPLETED")).status, 200);
      assert.equal(
        visit3d ? state.booking.payment_status : state.transaction.status,
        "completed",
      );
      assert.equal(state.queued.length, 0);
    });
  ...
  test(`${label} successful NOT_FOUND still finalizes a missing deposit`, async (t) => {
    if (visit3d) state.transaction = null;
    t.mock.method(globalThis, "fetch", async () =>
      Response.json({ status: "NOT_FOUND" }),
    );
    const result = await (await check(statusRequest())).json();
  ...
  test(`${label} NOT_FOUND cannot overwrite a concurrent completion`, async (t) => {
    if (visit3d) state.transaction = null;
...
test("first and repeated polls preserve a blocked daily completion", async (t) => {
  state.transaction.type = "property_lock";
  state.transaction.property_id = "property";
  state.property.period = "day";
  state.dailyBlocked = true;
  t.mock.method(globalThis, "fetch", async () =>
    Response.json({ status: "FOUND", data: { status: "COMPLETED" } }),
  );
  for (let poll = 0; poll < 2; poll++) {
    const result = await (await checkPaymentStatus(statusRequest())).json();
    assert.equal(result.status, "NEEDS_SUPPORT");
    assert.equal(result.raw.status, "NEEDS_SUPPORT");
  }
  assert.equal(state.transaction.status, "completed");
  assert.equal(state.successes.length, 0);
});

test("a fulfilled daily completion still returns success", async (t) => {
  state.transaction.type = "property_lock";
  state.transaction.property_id = "property";
  state.property.period = "day";
  t.mock.method(globalThis, "fetch", async () =>
  ...
  assert.equal(
    (await (await checkPaymentStatus(statusRequest())).json()).status,
    "COMPLETED",
  );
  assert.equal(state.successes.length, 1);
});

test("stored conflicts re-drive escalation without altering durable fulfillment", async () => {
  state.transaction.type = "property_lock";
  state.transaction.property_id = "property";
  ...
  const original = structuredClone(state.transaction);
  state.escalationError = true;
  assert.equal(
    (await (await checkPaymentStatus(statusRequest())).json()).status,
    "NEEDS_SUPPORT",
  );
  ...
  assert.deepEqual(state.transaction, original);
});
...
for (const [httpStatus, body] of [
  ...
  test(`3D gateway ${httpStatus} stays pending and accepts a later completion`, async (t) => {
    t.mock.method(globalThis, "fetch", async () => {
      const response = new Response(body, { status: httpStatus });
      if (body === null)
        response.text = async () => {
    ...
    assert.equal(result.depositId, state.booking.payment_deposit_id);
    ...
    assert.equal(callbackResult.error, undefined);
    assert.equal(state.booking.payment_status, "completed");
    assert.equal(state.queued.length, 0);
  });
}
