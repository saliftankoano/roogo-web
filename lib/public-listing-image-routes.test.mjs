  ...
  from(table) {
    ...
    const query = {
      ...
      maybeSingle: async () => {
        ...
        if (table === "room_types") return { data: state.room, error: null };
      ...
      then(resolve, reject) {
        ...
        if (operation === "insert") {
          const inputs = [value].flat();
          if (table === "property_images" && inputs.some(input =>
            /\/sha256-[0-9a-f]{64}\.(jpg|png|heic)$/.test(input.url) &&
            state.records.some(row => row.property_id === input.property_id && row.url === input.url))) {
            return Promise.resolve({ error: { code: "23505", message: "Duplicate content-addressed photo" } }).then(resolve, reject);
          }
          state.records.push(...inputs);
        }
...
test("overlapping identical requests cannot create duplicate rows", async () => {
  ...
  try {
    assert.equal(overlapping.status, 200, "the competing request can safely link the shared object");
  } finally {
    release();
  }
...
test("a failed database insert preserves an orphan and the retry recovers it", async () => {
  state.failInsert = true;
  assert.equal((await single.POST(request(photo("retry link")), context(propertyId))).status, 500);
  assert.equal(state.records.length, 0);
  assert.equal(state.objects.size, 1);
  assert.deepEqual(state.removed, []);
  // Recovery can itself fail: it must keep the object available for a later retry.
  assert.equal((await single.POST(request(photo("retry link")), context(propertyId))).status, 500);
  assert.equal(state.records.length, 0);
  assert.equal(state.objects.size, 1);
  assert.deepEqual(state.removed, []);
