...
export async function POST(
  ...
  try {
    ...
    if (agreement.renter_id !== user.id) return errorResponse("Forbidden: you are not the renter", 403, req);
    if (agreement.status !== "sent") return errorResponse("Agreement is not in sent status", 409, req);
