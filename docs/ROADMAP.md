...
- [ ] **Release clear, duplicate-safe payment failure notices** — customers can understand a failed charge and distinguish it from a paid reservation needing support. As of 2026-09-09, [web #29](https://github.com/saliftankoano/roogo-web/pull/29) and [mobile #29](https://github.com/saliftankoano/roogo/pull/29) are open, implemented PRs, not a verified production release.
  - **Done when:** the migration 070 duplicate-payment preflight passes; old payment-failure and conflict-notification workers are drained; payment migrations `068_payment_failure_notifications.sql`, `069_atomic_direct_property_lock.sql`, `070_single_use_listing_payments.sql`, `071_preserve_completed_property_locks.sql`, `072_payment_failure_send_boundary.sql`, and `073_conflict_notification_send_boundary.sql` are applied in that order; the backend is deployed before the companion client release. These filenames distinguish payment migrations from unrelated migrations sharing numeric prefixes.
  - **Additional migration gate:** apply `074_durable_listing_payment_consumption.sql` after 068–073 and before server deployment. Reconcile contradictory surviving property/transaction links before backfill. Review previously deleted paid listings from external evidence where consumption links were already lost; the migration cannot reconstruct absent history.
  - **Creation recovery gate:** apply `075_atomic_listing_amenities.sql` after 074 and before server deployment. It adds a nullable creation snapshot and an insert trigger; old rows are not rewritten. Verify a payment-link outage preserves the property and amenities and retry repairs both links. Historically missing amenities require original evidence, not automatic reconstruction from a changed retry.
  ...
  - **Dependencies:** [system behavior](./SYSTEM.md#how-do-failed-and-uncertain-customer-payments-recover) and [send-boundary decision](./DECISIONS.md#notification-uncertainty-never-authorizes-a-second-send--2026-09-08). Migrations 072/073 preserve legacy uncertainty; do not reset uncertain sends or rewrite historical fulfillment without provider evidence.
