-- property_details was created before is_test, dos_and_donts, etc. were added to properties.
-- PostgreSQL expands p.* at view creation time, so those columns were missing from the view
-- and PostgREST failed on .eq("is_test", false) with a column-not-found error.
